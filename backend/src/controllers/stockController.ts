...
export class StockController {
     ...
     */
    async checkStockExists(req: Request, res: Response): Promise<void> {
        try {
            const { symbol } = req.query;
            const symbolString = Array.isArray(symbol)
                ? typeof symbol[0] === "string" ? symbol[0] : undefined
            ...
            const exists = await stockRepository.checkStockExists(symbolString);
